...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (user.pro === true || user.todos.length < 10) {
    ...
    return response
      .status(403)
      .json({ error: "Update your plan to add more todos!" });
  }
}
...
function checksTodoExists(request, response, next) {
  ...
  if (!checkIdIsValid) {
    return response.status(400).json({ error: "Id informed is not valid!" });
  }

  const checkTodoExists = checkIfUserExists.todos.find(
    (todo) => todo.id === id
  );
